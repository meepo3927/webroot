...
;!function(){
    ...
    */
    ...
    Class.prototype.events = function(){
        ...
        //绑定呼出控件事件
        this.onShowEvent = function (elem) {
            that.bindElem = this;
            that.render();
        };
        options.elem.on(options.trigger, this.onShowEvent);
        //绑定关闭控件事件
        this.onDocumentClick = function (e) {
            if(e.target === options.elem[0] || e.target === lay(options.closeStop)[0]){
                return;
            }
            that.remove();
        };
        ...
        lay(document).on('click', this.onDocumentClick).on('keydown', this.onDocumentKeydown);
        ...
        lay(window).on('resize', this.onWinResize);
    ...
    Class.prototype.dispose = function () {
        ...
        // 移除事件绑定
        options.elem.off(options.trigger, this.onShowEvent);
        lay(document).off('click', this.onDocumentClick).off('keydown', this.onDocumentKeydown);
        lay(window).off('resize', this.onWinResize);
    };
